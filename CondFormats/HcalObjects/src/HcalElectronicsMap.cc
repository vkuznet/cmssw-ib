...
void HcalElectronicsMap::sortById () const {
  if (!std::is_sorted((*mPItemsById).begin(),(*mPItemsById).end(),hcal_impl::LessById())) {
...
void HcalElectronicsMap::sortByTriggerId () const {
  if (!std::is_sorted((*mTItemsByTrigId).begin(),(*mTItemsByTrigId).end(),hcal_impl::LessByTrigId())) {
